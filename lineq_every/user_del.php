...
<div id="cover">
	...
	<div id="main_contents">
		<form action=<?php echo "./user_del.php?id=".$id ?> method="post">
		...
		for($i = $start;$i < $end;$i++){
			...
			foreach($conteArr as $val) {
			  ...
			  if(pq($val)->find('.footer_answers')->text() != "回答0"){
					$HTMLData = file_get_contents('http://lineq.jp'.pq($val)->find('p')->find('a')->attr('href'));
					$phpQueryObj2 = phpQuery::newDocument($HTMLData);
					$conteArr1 = $phpQueryObj2['.reply_info_name'];
					if(preg_match('/板谷y/',$conteArr1->text())){

...
<div id="cover">
	...
	<div id="main_contents">
		<form action=<?php echo "./user_sp.php?id=".$id ?> method="post">
		<select name="kind">
		...
		開始:<input type="number" name="start" min = "1"><br>
		...
		for($i = $start;$i < $end;$i++){
			...
			foreach($conteArr as $val) {
			  ...
			  if(pq($val)->find('.footer_answers')->text() == "回答0"){
					$q_id = split("q/", pq($val)->find('p')->find('a')->attr('href'));
					echo "<div class='cards'>";

				  echo "<a href='lineq://detail?q=".$q_id[1]."&from=spweb&locale=JP' target='blank'>";
				  echo pq($val)->find('p')->text();
				  echo "<br><div class='btn'>回答する</div>";
				  echo "</a>";
				  echo "</div><br>";
			  }
			}
